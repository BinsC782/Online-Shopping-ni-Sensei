/**
 * API Service for handling all API requests with built-in caching, retry logic, and error handling
 * @class ApiService
 */
class ApiService {
    // API Configuration
    ...
    static async #fetchWithRetry(options, retryCount = 0) {
        ...
        const requestOptions = { ...options, retryCount };
        ...
        if (!navigator.onLine) {
            return new Promise((resolve, reject) => {
                this.#queueRequest({ ...options, resolve, reject });
            });
        }
    ...
    static #calculateRetryDelay(retryCount, error) {
        ...
        return baseDelay * (0.5 + Math.random() * 0.5); // Add jitter
    }
    ...
    static async addToCart(productId, quantity = 1, { merge = true } = {}) {
        ...
        try {
            ...
            const previousCart = this.#getFromCache('user_cart') || { items: [] };
            const existingItem = previousCart.items.find(item => item.productId === productId);
            
            if (existingItem) {
                const newQuantity = merge ? existingItem.quantity + quantity : quantity;
    ...
    static async updateCartItem(productId, quantity) {
        ...
        try {
            ...
            // Invalidate cart and product caches
            this.#cache.delete('user_cart');
            this.#cache.delete(`product_${productId}`);
    ...
    static updateCartUI(cartData) {
        ...
        if (cartBadge) {
            const itemCount = cartData?.items?.reduce((total, item) => total + (item.quantity || 0), 0) || 0;
            cartBadge.textContent = itemCount > 99 ? '99+' : itemCount;
            cartBadge.style.display = itemCount > 0 ? 'flex' : 'none';
            cartBadge.setAttribute('aria-hidden', itemCount > 0 ? 'false' : 'true');
        }
     ...
     * @private
     */
    static #renderCartPage(cartData) {
        ...
        container.innerHTML = `
            <div class="cart-page">
                ...
                <div class="cart-summary">
                    ...
                    <div class="summary-row">
                        <span>Shipping</span>
                        <span>${shipping === 0 ? 'Free' : `$${shipping?.toFixed(2) || '0.00'}`}</span>
                    </div>
                    <div class="summary-row">
                        <span>Tax</span>
        ...
        container.querySelectorAll('.quantity-control').forEach(control => {
            control.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    // Refresh cart
                    const cart = await this.getCart(true); // Force refresh
    ...
    static #renderCartItem(item) {
        ...
        return `
            <div class="cart-item" data-item-id="${this.#escapeHtml(productId)}">
                ...
                <div class="item-details">
                    <h3 class="item-name">${this.#escapeHtml(name)}</h3>
                    <p class="item-price">$${Number(price).toFixed(2)} each</p>
                    
                    <div class="quantity-controls">
                        <button type="button" 
                                class="quantity-control decrease" 
                        ...
                        <input type="number" 
                               ...
                               aria-label="Quantity">
                        <button type="button" 
                                class="quantity-control increase" 
